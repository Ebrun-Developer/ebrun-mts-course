#!/usr/bin/env python3
"""
ebrun-mts-course - 检查 Skill 版本更新
...
HEADERS = {
    "User-Agent": "Mozilla/5.0 (EbrunMtsCourseUpdate/1.0)",
...
def is_safe_url(url: str) -> bool:
    try:
        ...
        return any(
            ...
            or (parsed.hostname and parsed.hostname.endswith("." + domain))
...
def read_local_config() -> Dict[str, Any]:
    try:
        ...
        raise UpdateCheckError(f"未找到本地配置文件: {CONFIG_FILE}", EXIT_NOT_FOUND) from error
    except json.JSONDecodeError as error:
        raise UpdateCheckError(f"本地 config.json 解析失败: {error}", EXIT_JSON_ERROR) from error

    if not isinstance(data, dict):
        raise UpdateCheckError("本地 config.json 格式异常: 顶层必须是对象", EXIT_JSON_ERROR)

    meta = data.get("_meta")
    if not isinstance(meta, dict):
        raise UpdateCheckError("本地 config.json 缺少 _meta 对象", EXIT_JSON_ERROR)

    current_version = str(meta.get("version", "")).strip()
    if not current_version:
        raise UpdateCheckError("本地 config.json 缺少 _meta.version", EXIT_JSON_ERROR)
    ...
    if not isinstance(interval_hours, (int, float)) or interval_hours <= 0:
        raise UpdateCheckError("本地 config.json 中 check_interval_hours 必须大于 0", EXIT_JSON_ERROR)
...
def build_cached_result(
    ...
    return {
        ...
        "update_available": parse_bool(
            cache_info.get("last_update_available"),
...
def build_repo_config_file_urls(repo_url: str) -> List[str]:
    ...
    if repo_info["provider"] == "github":
        candidates.extend(
            [
                f"https://raw.githubusercontent.com/{owner}/{repo}/{branch}/references/config.json"
        ...
        candidates.extend(
            [
                f"https://gitee.com/{owner}/{repo}/raw/{branch}/references/config.json"
                for branch in branches
            ]
        )
...
def fetch_repo_version_info(local_info: Dict[str, Any], timeout: int, retries: int) -> Dict[str, Any]:
    ...
    for source_name, repo_url in repo_sources:
        ...
        candidate_urls = build_repo_config_file_urls(repo_url)
        if not candidate_urls:
            errors.append(f"{source_name}: 无法从仓库地址推导 references/config.json 地址")
            continue

        for candidate_url in candidate_urls:
            try:
                remote_info = fetch_json(candidate_url, timeout, retries)
                remote_meta = remote_info.get("_meta")
                if not isinstance(remote_meta, dict):
                    raise UpdateCheckError("远端 config.json 缺少 _meta 对象", EXIT_JSON_ERROR)
                remote_current_version = str(remote_meta.get("version", "")).strip()
                if not remote_current_version:
                    raise UpdateCheckError("远端 config.json 缺少 _meta.version", EXIT_JSON_ERROR)
    ...
    raise UpdateCheckError(
        "；".join(errors) if errors else "无法从远端仓库读取 references/config.json",
        EXIT_REQUEST_ERROR,
    )
...
def build_fallback_result(
    ...
    try:
        ...
        message = (
            f"版本接口不可用，已降级到远端仓库 config.json；检测到版本不一致: {current_version} -> {latest_version}"
            if update_available
            else "版本接口不可用，已降级到远端仓库 config.json；当前未发现版本变化"
        )
        ...
        return {
            ...
            "message": "版本接口不可用，且无法从远端仓库读取 references/config.json，当前无法判断是否有新版本",
        }
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="检查 ebrun-mts-course Skill 是否有新版本",
